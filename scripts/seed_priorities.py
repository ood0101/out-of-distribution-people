...
SEED = {
    ...
    "siddharth-karamcheti": {
        "urgency_reason": "robotics-VLA / academic anchor. Stanford CS PhD completed 2025 (co-advised Percy Liang + Dorsa Sadigh = CLAUDE.md critical lab). OpenVLA co-author (CoRL 2024 Outstanding-Paper finalist; 2,682 cites, 6.3K GitHub stars = the de-facto open 7B VLA, the open alternative to closed pi-0 / Figure Helix / RT-2; OpenVLA beat 55B RT-2-X at 7B). FIRST author Voltron / Language-Driven Representation Learning (RSS 2023 Best-Paper finalist, 250 cites) + Prismatic VLMs (331 cites, 986 stars, the backbone OpenVLA was built on). ~17,448 total cites / h-23 / i10-28 (headline inflated by ~100-author Foundation Models report at 10,151; core profile is OpenVLA + Voltron + Prismatic). INTERN (not staff) at TRI Large Behavior Models (Russ Tedrake org) + Hugging Face. Brown BS (CS + Literary Arts; Charniak + Tellex). Open Philanthropy AI Fellow. Indian-origin (Telugu, name-inferred). THE INFLECTION IS RESOLVED AND IT'S ACADEMIC: incoming Assistant Professor of Interactive Computing at Georgia Tech (Fall 2026), actively recruiting PhD students = cleanest anti-founding signal -> honestly T2 not T1. But track, not discard: his co-authors Chelsea Finn + Sergey Levine are sitting professors who co-founded Physical Intelligence ($2.4B) = the professor-founder optionality template (2-4yr horizon). High-value cluster hub: incoming lab = future T0/T1 student pipeline; bridges Stanford (Liang/Sadigh) <-> Berkeley (Levine) <-> TRI <-> OpenVLA team. robotics-VLA cluster anchor: Kushal Kedia (T0) + Weiying Wang (T0) + Ritvik Singh (T1) + Himanshu Gaurav Singh (T1); Percy Liang bridge to Tanishq Kumar (T2). T2 relationship-build, quarterly cadence, professor-founder fuse.",
        "urgency_decay_date": None,
        "channel": "email",
        "next_action": "Email skaramcheti@gatech.edu (published GT address; fallback X DM @siddkaramcheti). Subject: OpenVLA's open lane vs pi-0 + your GT lab. Congratulate the Georgia Tech faculty role (do NOT ask 'what's next post-PhD' — that's answered). Hook on his OWN 1st-author Voltron insight (language as supervisory signal for robot vision, not just goal-conditioning), name the pi-0 / Figure Helix / Gemini Robotics landscape to prove market understanding, invoke the Finn/Levine -> Physical Intelligence professor-founder pattern to signal long-horizon intent. Offer cluster intel + student-pipeline value, NO pitch, NO capital ask. AUTHORSHIP PRECISION: OpenVLA 3rd-listed lead-block author (NOT 1st = Moo Jin Kim, NOT senior PI = Levine/Finn); 1st author on Voltron + Prismatic. TRI/HF were internships not staff roles — do not overstate. RECURSIVE DISCOVERIES TO INDEX: Moo Jin Kim (P1, OpenVLA 1st author, Stanford — higher-priority spinout-watch than Sidd) + Karl Pertsch (P1, OpenVLA 2nd author, DROID / Open X-Embodiment lead, Berkeley/Stanford) + Suvir Mirchandani + Jennifer Grannen (P2, Sadigh-lab cohort).",
        "cluster": "robotics",
    },
    "suraj-nair": {
        "urgency_reason": "robotics-VLA cluster anchor + departure-watch. FOUNDING-TEAM Research Scientist at Physical Intelligence ($5.6B; pi-0 / pi-0.5; founders Karol Hausman, Sergey Levine, CHELSEA FINN, Brian Ichter, Lachy Groom) = the category leader in robot foundation models. SOLE first author on R3M (CoRL 2022, 1,006 cites; ICRA Scaling-Robot-Learning Best Paper) — freeze a representation pretrained purely on Ego4D human video (time-contrastive + video-language alignment + L1) and downstream robot policy learning gets +20% task success vs scratch / +10% vs CLIP/MoCo, Franka arm learns real-apartment manipulation from 20 demos. One of the foundational pre-VLA representation papers (alongside MVP/Voltron); downloadable checkpoint at facebookresearch/r3m made it a default backbone. Stanford CS PhD 2018-2023 co-advised by Chelsea Finn + Silvio Savarese (thesis 'Scaling Deep Robotic Learning to Broad Real-World Data'). Caltech BS 2014-2018 GPA 3.9, multi-agent RL with Yisong Yue. RSS Pioneer 2022 (top-30 early-career roboticists). NSF GRFP; Apple ML/AI Fellowship nominee. Prior: TRI Research Scientist 2023-2024, FAIR, Google Brain, GE. CITATION HONESTY: Scholar 22,680 / h-29 / i10-33 is consortium-inflated (Foundation Models report ~10,167; OpenVLA / DROID / Open-X / pi-0 are middle-author consortium papers) — real personal spike is R3M (sole 1st) + first-author tail (Hierarchical Foresight, Goal-Aware Prediction); RoboNet is a contributing-author role (4th of 9), not a co-lead. TWO PI-FOUNDER CO-AUTHORSHIP TIES: Chelsea Finn (advisor) + Brian Ichter (CoRL 2021) = inside the founding lineage. WHY T2 NOT HIGHER: no founding signal — founding-team at the $5.6B leader = deep early-equity lock-in + best job in the field = opposite of departure setup; R3M is a single foundational result now shared territory (MVP/Voltron/VC-1/VLA wave); academia is a live alt path (cf. Karamcheti -> Georgia Tech). THE HOOK: the day he leaves PI to found, instant T0 (foundational first-author paper + founding-team operating reps + Caltech/Stanford). Direct repo co-author with Siddharth Karamcheti (OpenVLA, Prismatic, Voltron); cluster bridges Pannag Sanketi (T0) + Kushal Kedia + Ritvik Singh + Himanshu Gaurav Singh + Weiying Wang. Indian-origin (Malayali). NOTE: pi-0.5 named-authorship NOT verified (pi-0 byline confirmed via CV); @SurajNair_1 auth-gated, could not pull live tweets.",
        "urgency_decay_date": None,
        "channel": "email",
        "next_action": "Email surajnair0220@gmail.com (fallback X DM @SurajNair_1). Subject: 'R3M's frozen-representation bet vs. the VLA wave.' Lead with his SOLE first-author R3M result (not the inflated 22K total), pose the real technical tension (decoupled frozen R3M-style pretraining vs jointly-trained end-to-end VLAs like pi-0 / OpenVLA / Gemini Robotics — does the frozen-backbone idea still earn its keep at pi-0/0.5 scale, or did flow-matching over a jointly-trained VLM swallow it?), name the landscape (Skild, Figure/1X, Generalist) to prove market understanding. NO pitch, NO capital ask — he is founding-team at PI and not raising; plant the relationship so we're in the inbox the day he leaves. RECURSIVE DISCOVERIES TO INDEX: Aravind Rajeswaran + Vikash Kumar + Abhinav Gupta (R3M co-authors, FAIR robot-representation cluster) + Silvio Savarese lab + PI founding team.",
        "cluster": "robotics",
    },
    "scott-reed": {
        "urgency_reason": "ONE OF THE MOST SENIOR RESEARCHER-DEPARTURES IN THE REPO + explicit founding signal. Site + LinkedIn both say 'building something new' / 'Something new' (Atlanta Metro). Just left NVIDIA (one of 20 tracked) where he was PRINCIPAL RESEARCH SCIENTIST + FOUNDING MEMBER of the GEAR lab (Generalist Embodied Agent Research; Jim Fan + Yuke Zhu), 2024-2026; before that ~8yrs at Google DeepMind (also one of 20 tracked) as SENIOR STAFF RESEARCH SCIENTIST. LEAD author of GATO / 'A Generalist Agent' (DeepMind 2022, 1,631 cites) = the single-network-same-weights agent across 600+ tasks (Atari/captioning/dialogue/real-robot control), the canonical generalist-agent statement that prefigured the 2024-26 agent wave. ALSO FIRST author of Generative Adversarial Text-to-Image Synthesis (ICML 2016, 4,811 cites) = one of the first text-to-image papers, 4yrs pre-DALL-E, pre-diffusion. PhD U Michigan 2016, advisor Honglak Lee. PEDIGREE: genuine co-author on GoogLeNet (ILSVRC 2014 winner ~73K) + SSD (~55K) from Google-intern era. CITATION HONESTY: ~150K Scholar / h-51 (3rd-party index ~86K, treat as approximate) is dominated by middle-author GoogLeNet+SSD; the personal spike is the TWO first/lead-author foundational papers (GATO + text-to-image GAN) bookending the generative-AI era. BRIEF CORRECTIONS: Parti author list does NOT include Reed (verified); Flamingo unconfirmed, omitted. STEALTH CO NOT YET PUBLIC (highest-value open question): no Crunchbase / SEC Form D / domain / press / co-founder as of May 2026; only the 'building something new' flip + Atlanta Metro (unusual for frontier-AI founder, flagged not theorized). DO NOT CONFLATE with the Mar 2025 TechCrunch 'NVIDIA-backed DeepMind robotics stealth' / Apr 2026 SiliconANGLE 'GEN-1' stories = both are Pete Florence / Generalist AI (verified directly). THESIS PREDICTION: most likely an agent foundation-model / embodied-AI / world-model company (commercial descendant of GATO = one generalist policy across digital + physical). LANDSCAPE: Physical Intelligence ($5.6B), Generalist AI (Pete Florence, NVIDIA-backed, same lab lineage = the precedent), World Labs (Fei-Fei Li), Figure/1X, Decart, Gemini Robotics / NVIDIA GR00T. RISKS: (1) extremely competitive raise = a GATO lead author from NVIDIA GEAR + DeepMind will be pre-empted; must be in inbox before company public; (2) 'building something new' doesn't strictly prove FOUNDER vs joining/advisor (resolve via WHOIS/CA & GA SoS/Form D); (3) capital-intensive category; (4) first-time founder. NETWORK/RECURSIVE: Jim Fan (Exceptional 100, GEAR co-lead = warmest intro), Yuke Zhu (GEAR), Honglak Lee (Michigan advisor + alumni cluster), GATO co-authors (Zolna/Parisotto/Colmenarejo/Novikov/de Freitas/Vinyals/Hadsell).",
        "urgency_decay_date": None,
        "channel": "twitter",
        "next_action": "DM @scott_e_reed (fallback LinkedIn reedscott); STRONGEST path = warm intro via Jim Fan (Exceptional 100, GEAR co-lead). Subject: 'GATO's generalist thesis, now as a company.' Lead with the GATO thesis (QUOTE it: same network/same weights deciding from context whether to emit text/joint-torques/button-presses), pose the digital-action-vs-embodied generalist-agent fork, name the PI / Generalist AI / World Labs / Figure landscape to prove market fluency, offer $500K-$1M and speed, NO 'pick your brain.' Sign Vansh. RESOLVE: founder-vs-joiner + company identity via WHOIS/CA & GA SoS/SEC Form D. RECURSIVE DISCOVERIES TO INDEX: Yuke Zhu (GEAR co-lead, UT Austin), GATO co-authors (Konrad Zolna, Emilio Parisotto, Sergio Gomez Colmenarejo, Alexander Novikov, Jake Bruce, Ashley Edwards = generalist-agent team, co-departure/co-founder watch), Honglak Lee Michigan generative-models alumni.",
        "cluster": "generalist-agents",
    },
    "siddhant-haldar": {
        "urgency_reason": "CLEAN T0 FOUNDING MOMENT, DOUBLE-SOURCED. Own resume lists top entry verbatim 'Stealth Startup | Co-Founder & CTO | Dec 2025-Present'; Lerrel Pinto's lab page independently lists him as 'co-founder of a stealth robotics company.' Just finished PhD at NYU Courant CILVR Lab (2021-2025) under LERREL PINTO = one of the strongest robotics-founder labs in academia (Denis Yarats->Perplexity CTO, Pinto->Fauna->Amazon Mar26 + ARI->Meta May26, Ademi Adeniji->Grounded Superintelligence, Raunaq Bhirangi->stealth robotics co). FIRST AUTHOR on the data-efficiency award line: ROT/'Watch and Match' (CoRL 2022 Oral, Best Paper Finalist, ~116c) = optimal-transport reward matching for fast reward-free imitation; FISH/'Teach a Robot to FISH' (RSS 2023 Best Student Paper, ~117c) = manipulation from ~1 MINUTE of demos; BAKU (NeurIPS 2024, sole 1st, ~117c) = efficient multi-task policy transformer; Point Policy (CoRL 2025, sole 1st, ~45c) = human-to-robot transfer with ZERO robot teleop. Co-author Open X-Embodiment (ICRA 2024 Best Paper). THROUGHLINE (dissertation 'Data-Efficient Generalization for Robotics'): algorithmic sample-efficiency -> efficient architectures -> ELIMINATING ROBOT DATA via human demos (key points / egocentric video / tactile: Point Policy, Point Bridge, EgoZero, Feel the Force). = the robot-data-bottleneck thesis the whole VLA wave is funding. IIT Kharagpur Dual Degree EE (CGPA 9.08; CS minor 9.63); Software Team Lead Team AGV (2nd/43 IGVC 2018). Interns: NVIDIA Seattle Robotics Lab (Mandlekar/Narang, from Jun25), Nuro behavior-ML (2023); UW visiting (Fox/Gupta, 2025). INDIAN-ORIGIN (Garden High Kolkata; hometown Kolkata inferred). CITATION HONESTY: 2,461 total / h-12 dominated by Open X-Embodiment (~1,684, large collab); real first-author spike ~400c. COMPANY NOT PUBLIC as of Jun 2026: no name/Crunchbase/Form D/funding signal = pre-name inception opportunity. CO-FOUNDER HYPOTHESIS (strong, unconfirmed): Raunaq Bhirangi (NYU postdoc, identical 'stealth robotics co' descriptor, AnySkin tactile sensors, Haldar's most frequent recent co-author; complementary policy/IL vs tactile-hardware). DO NOT CONFLATE with Pinto's own Fauna/ARI exits (separate ventures). RISKS: (1) fully stealth, thesis inferred; (2) first-time founder, CTO not CEO (separate CEO co-founder likely); (3) capital-intensive crowded category (PI $5.6B, Skild, Generalist); (4) NVIDIA intern line overlaps Dec25 founder line, confirm full-time.",
        "urgency_decay_date": None,
        "channel": "twitter",
        "next_action": "DM @haldar_siddhant (fallback email siddhanthaldar@nyu.edu, may bounce post-grad; warm intro via Pinto orbit strongest). Subject: 'FISH -> Point Policy, and the data bottleneck as a company.' Lead with the FISH->Point Policy PROGRESSION (1-min demos -> no-teleop human-to-robot) read as a company thesis, pose the data-collection-moat-vs-architecture-moat fork, name the PI / Skild / Generalist / human-demo-to-robot landscape, offer $500K-$1M + speed, NO 'pick your brain.' Sign Vansh. RESOLVE: company name + co-founder roster (is it Raunaq Bhirangi?) + full-time status, via direct DM / WHOIS / SoS / Form D. RECURSIVE DISCOVERIES TO INDEX: Raunaq Bhirangi (P0, likely co-founder, @raunaqmb), Ademi Adeniji (Grounded Superintelligence, @AdemiAdeniji), Lerrel Pinto (track next spinout), Jeff/Zichen Cui (DynaMo 1st author, current CILVR PhD), Jyo Pari (FISH co-1st, now MIT), Irmak Guzey (CILVR tactile).",
        "cluster": "robotics-vla",
    },
}
