...
QUEUE_FILE = REPO / "queue.html"   # the outreach queue now lives on its own page
...
<head>
  ...
  <title>Outreach Queue &mdash; Out of Distribution People</title>
  <link rel="stylesheet" href="style.css">
  <style>
    .q-top { position: sticky; top: 0; z-index: 50; background: var(--bg);
      border-bottom: 1px solid var(--border); margin: 0 -2rem 1.5rem; padding: 0.7rem 2rem; }
    .q-top-row { display:flex; align-items:baseline; justify-content:space-between; gap:1rem; flex-wrap:wrap; }
    .q-title { font-family: var(--mono); font-size: 0.8rem; text-transform: uppercase; letter-spacing: 0.12em; color: var(--fg-muted); }
    .q-links { display:flex; gap:1.1rem; font-family: var(--mono); font-size:0.72rem; text-transform:uppercase; letter-spacing:0.08em; }
    .q-links a { color: var(--fg-muted); text-decoration:none; padding-bottom:0.15rem; border-bottom:1px solid transparent; }
    .q-links a:hover, .q-links a.cur { color: var(--accent); border-bottom-color: var(--accent); }
    @media (max-width:600px){ .q-top { margin:0 -1rem 1rem; padding:0.6rem 1rem; } }
  </style>
</head>
<body>
  <div class="q-top">
  ...
  <h1 style="margin-bottom:0.2rem;">Outreach Queue</h1>
  <p class="subtitle" style="margin-top:0;">Who to reach today &mdash; seeded urgency, ranked by tier &times; decay. Auto-built from the live state; run the morning ritual to refresh.</p>
"""

PAGE_FOOT = """
  <footer style="margin-top:2rem;">
    auto-generated by data/build_outreach_view.py from data/outreach_state.json &middot;
...
def main() -> int:
    ...
    page = PAGE_HEAD + panel_html + PAGE_FOOT
    prior = QUEUE_FILE.read_text(encoding="utf-8") if QUEUE_FILE.exists() else ""
    if page == prior:
        print("No change — queue.html already current.")
        return 0

    QUEUE_FILE.write_text(page, encoding="utf-8")
    updated = page  # for the byte-delta report below
    ...
    print(f"Wrote outreach queue to {QUEUE_FILE.name} ({len(updated)} bytes)")
